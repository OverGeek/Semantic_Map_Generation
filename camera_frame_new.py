...
def compute_coord_robot(imgLp, imgRp, j, corners, corner_map, coord, file1):
...
def main(seq=300):
    images = glob.glob("E:\Sem 7 Project\place_recognition\Complete_dataset\image_0/*.png")
    images.sort()
    count = 0
    obj_det = []
    ...
    obj_detect_coordinates = f"./Hypothesis_{Hypothesis_no}/annot_depths_hypoth{Hypothesis_no}_seq22.txt"
    output_file = f"./Hypothesis_{Hypothesis_no}/camera_frame_set_1_hypoth{Hypothesis_no}_seq22.txt"
    ...
    for index in range(len(images)):
        print(images[index][-14:])
        for corners in obj_det:
            # print(corners["ID"])
            if corners["ID"] == images[index][-14:]:
                ...
                right_image_path = "E:\Sem 7 Project\place_recognition\Complete_dataset\image_1/right" + images[index][-10:]
                corner_sequence = [id, name, [x1, y1], [x2, y2], [x3, y3], [x4, y4]]
                compute_coord_robot(left_image_path, right_image_path, corner_sequence, corners, corner_map, coord, file1)
