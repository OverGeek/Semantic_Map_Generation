...
def compute_depth(depthMap, uvL, uvR, k):
    ...
    #right cam params
    rk = [1399.24, 1399.24, 898.46, 503.932]
    ...
    for i in range(len(uvL)):
        ...
        if disp1 > 0:
            #TODO: create different hypothesis
            alpha = 1/disp1
            offset = abs(k[2]-rk[2])*alpha
            #d1 = k[0] * k[4] / disp1
            d1 = k[0] * k[4] / (disp1-offset)
            depth1.append(d1)
...
def main():
    ...
    pathL = 'E:\Sem 7 Project\place_recognition\Complete_dataset\image_0'
    pathR = 'E:\Sem 7 Project\place_recognition\Complete_dataset\image_1'

    rectify = False
    depth_dict = {}

    for img_name_l, img_name_r in zip(os.listdir(pathL)[:1700], os.listdir(pathR)[:1700]):
        print(img_name_l, img_name_r)

        # if L and R images are available as combined images
        ...
        # if L and R images are seperate
        imgL = cv2.imread(os.path.join(pathL, img_name_l))
        imgR = cv2.imread(os.path.join(pathR, img_name_r))
    ...
    with open(f'./depths_seq22.json', 'w') as f:
